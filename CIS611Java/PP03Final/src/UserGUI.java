...
public class UserGUI extends JPanel {
	...
	private static JPanel employeePane, addressPaneMaster, addressPane1, addressPane2, datePane1, datePane2,
			...
			payHeader, outputPane, textPane, controlPane, totalsPane;
	private static JComboBox<String> dayList1, monthList1, yearList1, dayList2, monthList2, yearList2, empList;
	private static JTextField street, houseNum, city, state, zip, rID, pID, multi1, multi2, eID, firstName, lastName, totalPay, avgPay;
	...
	public UserGUI() {
		...
		while (loop) {
			String temp = JOptionPane.showInputDialog(null, "Number of pay records to add");
			if (temp == null)
				System.exit(0);
			try {
		...
		addEmployeeButton.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (!employeeCheck()) {
					JOptionPane.showMessageDialog(null, "Please fill out all employee fields");
					return;
				}
				// set the full time / hourly status
				Status status = Status.FullTime;
				if (empList.getSelectedItem().toString() == "Hourly")
					status = Status.Hourly;
				int eIDint = Integer.parseInt(eID.getText());
				// create the address object
				Address tempAddress = new Address(street.getText(), houseNum.getText(), city.getText(), state.getText(),
						Integer.parseInt(zip.getText()));
				// pass everything to the employee creator
				payRoll.createEmployee(firstName.getText(), lastName.getText(), tempAddress, eIDint, status);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!payrecordCheck()) {
					JOptionPane.showMessageDialog(null,
							"Please fill out all pay record fields and the employee ID field");
					return;
				}

				// check that the date ranges make sense
				int status = 0;
				if (empList.getSelectedItem().toString() == "Hourly")
					status = 1;
				Date startDate = getStartDate();
				Date endDate = getEndDate();
				if (status == 1 && payRoll.getDateDiff(startDate, endDate) < 1) {
					JOptionPane.showMessageDialog(null,
							"Date range invald. Please make sure the date range spans one full day");
					return;
				} else if (status == 0) {
					long numMonths = payRoll.getDateDiff(startDate, endDate) / 28;
					if (numMonths < Long.parseLong(multi2.getText())) {
						JOptionPane.showMessageDialog(null,
								"Date range invald. Please make sure the date range spans the number of months entered");
						return;
					}
				}

				// create record
				int rIDint = Integer.parseInt(rID.getText());
				int eIDint = Integer.parseInt(eID.getText());
				int pIDint = Integer.parseInt(pID.getText());
				if (empList.getSelectedItem().toString() == "Hourly")
					status = 1;
				payRoll.createPayRecord(status, rIDint, eIDint, pIDint, startDate, endDate, multi1.getText(),
						multi2.getText());
			}
		});
	...
	private void initGUI() {
		...
		totalPay = new JTextField(15);
		avgPay = new JTextField(15);
	...
	private void doTheLayout() {
		...
		totalsPane.add(new JLabel("Total Pay"));
		totalsPane.add(totalPay);
		totalsPane.add(new JLabel("Average Pay"));
		totalsPane.add(avgPay);
		outputPane.add(controlPane, "North");
		outputPane.add(textPane, "Center");
		outputPane.add(totalsPane, "South");
	...
	public static void updateTextArea(String output) {
		if (textArea.getText().trim().isEmpty()) {
			...
			textArea.setText(textArea.getText() + System.lineSeparator() + System.lineSeparator() + output);
		}
	}

	public static void updateTotals(String total, String average) {
		totalPay.setText(total);
		avgPay.setText(average);
	}
	
public static Date getStartDate() {
		Date date = null;
		SimpleDateFormat formatter = new SimpleDateFormat("dd-MM-yyyy");
		String dateString = dayList1.getSelectedItem() + "-" + monthList1.getSelectedItem() + "-"
				+ yearList1.getSelectedItem();
		try {
	...
	public static Date getEndDate() {
		Date date = null;
		SimpleDateFormat formatter = new SimpleDateFormat("dd-MM-yyyy");
		String dateString = dayList2.getSelectedItem() + "-" + monthList2.getSelectedItem() + "-"
				+ yearList2.getSelectedItem();
		try {
			date = formatter.parse(dateString);
		} catch (ParseException e1) {
			// TODO Auto-generated catch block
	...
	public static boolean employeeCheck() {
		// returns false if any employee fields are empty, true otherwise
		if (eID.getText().trim().isEmpty() || firstName.getText().trim().isEmpty()
				|| lastName.getText().trim().isEmpty() || street.getText().trim().isEmpty()
				|| houseNum.getText().trim().isEmpty() || city.getText().trim().isEmpty()
	...
	public static boolean payrecordCheck() {
		// returns false if any pay record fields are empty, true otherwise
		if (pID.getText().trim().isEmpty() || multi1.getText().trim().isEmpty() || multi2.getText().trim().isEmpty()
				|| rID.getText().trim().isEmpty() || eID.getText().trim().isEmpty()) {
			return false;
		} else {
			return true;
		}
