...
public class Validation {
  ...
  private boolean prefixCheck(long number, int d) {
      ...
      else return false;  //d is an invalid prefix (longer than 2 chars)
      // loop to remove rightmost digits until only d digits remain
      while (number > iterator) {
          number = Long.divideUnsigned(number, divisor);
      }
      if ((Long.valueOf(number).intValue()) == d)    
          return true;
      return false; //number contains invalid prefix
